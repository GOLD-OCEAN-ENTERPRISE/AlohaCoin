...
// AlohaCoin message start (switch from PeerCoin's in v0.4)
static unsigned char pchMessageStartBitcoin[4] = { 0xf9, 0xbe, 0xb4, 0xd9 };
static unsigned char pchMessageStartAlohaCoin[4] = { 0x77, 0x77, 0x36, 0x98 };
...
void GetMessageStart(unsigned char pchMessageStart[], bool fPersistent)
{
    if (fTestNet)
        ...
        memcpy(pchMessageStart, (fPersistent || GetAdjustedTime() > nMessageStartSwitchTime)? pchMessageStartAlohaCoin : pchMessageStartBitcoin, sizeof(pchMessageStartAlohaCoin));
}
